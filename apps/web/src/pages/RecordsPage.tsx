...
import { cx, pageStackClass, primaryButtonClass, secondaryButtonClass, softCardClass } from '../components/ui';
        ...
        <div className="grid gap-3.5">
          {items.length === 0 && !loading ? (
            <div className={`${softCardClass} grid gap-3 py-6 text-center`}>
              ...
              <p className="m-0 text-sm leading-6 text-[#6b5f59]">完成第一张头像后，进度、结果和再次生成入口都会出现在这里。</p>
...
function RecordCard({ item, index, onOpen, onRetry, onRegenerate, onPreview }: RecordCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-2.5">
        {item.status === 'completed' ? (
          <button type="button" className={secondaryButtonClass} onClick={() => onOpen(item)}>
            查看结果
          </button>
        ) : showProgress ? (
          <button type="button" className={secondaryButtonClass} onClick={() => onOpen(item)}>
        ...
        {item.status === 'failed' ? (
          <button type="button" className={secondaryButtonClass} onClick={() => onRetry(item)}>
            重试任务
          </button>
        ) : null}
        {!showProgress ? (
