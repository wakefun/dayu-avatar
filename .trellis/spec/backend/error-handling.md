...
- `POST /api/generation-tasks/:taskId/fine-tune` accepts `{ prompt: string }` for a completed, user-owned task with a generated result. It derives `generation_results.image_asset_id` server-side, requires that asset to be a user-owned `generated_result`, creates a new task with that asset as the sole source image, no style references, source task model/quality/size/output format, and `sourceTaskId` set to the source task id.
- `POST /api/generation-tasks/:taskId/cancel` accepts only queued/processing tasks, marks them canceled plus soft-deleted, and normal task/queue/history/record endpoints then treat them as not found/hidden.
- `DELETE /api/records/:taskId` accepts terminal records, soft-deletes the task, soft-deletes any active gallery item for its result, clears `generation_results.saved_to_gallery`, and never deletes static files.
- `DELETE /api/gallery-items/:itemId` soft-deletes the gallery item and clears `saved_to_gallery`; saving the same result later reactivates the existing unique row.
- Asset/result/record payloads expose `contentHash` when the backing `file_assets.content_hash` exists.
...
| Fine-tune prompt is empty after trim | Return `400 VALIDATION_ERROR` |
| Fine-tune source task is missing or not owned | Return `404 NOT_FOUND` |
| Fine-tune source task is not completed | Return `409 INVALID_STATE` |
| Fine-tune source task has no result or unavailable generated image | Return `404 NOT_FOUND` |
| Fine-tune result image asset is not `generated_result` | Return `409 INVALID_STATE` |
...
| Cancel endpoint receives completed/failed/canceled task | Return `409 INVALID_STATE` |
| Record delete receives queued/processing task | Return `409 INVALID_STATE`; caller should cancel first |
| Task was canceled or soft-deleted | Return `404 NOT_FOUND` from normal owned-task lookup routes |
| Gallery item was removed or its record was soft-deleted | Return `404 NOT_FOUND` from avatar/download/update routes |
...
- Good: cancel/delete/gallery-remove flows use soft-delete updates and keep uploaded/generated static files on disk.
- Good: record delete cleans up active gallery visibility for that result before returning success.
- Good: asset creation computes and returns `contentHash`; duplicate uploads/generated bytes reuse existing storage paths instead of writing duplicate files.
...
- Good: fine-tune creates a derived task through the scoped source-task endpoint instead of exposing generated asset ids to the frontend or relaxing generic task creation category validation.
...
- Bad: do not allow `generated_result` asset ids through the generic `POST /api/generation-tasks` upload-reference validation; keep that exception scoped to fine-tune.
...
- Bad: do not return canceled or soft-deleted tasks from queue/history/records just because their status is terminal.
- Bad: do not physically delete static media in user-visible delete flows; deleted database rows should be marked instead.
...
- Add assertions for `POST /api/generation-tasks/:taskId/fine-tune`: empty prompt validation, active/non-completed task invalid-state, derived task `sourceTaskId`, sole generated-result source asset, empty style refs, and reused generation params.
...
- Add API smoke assertions for cancel hiding, record/gallery soft deletion, gallery re-save reactivation, asset content-hash exposure, duplicate storage-path reuse, and static file retention.
...
```ts
// Generic task creation should not accept generated_result as a normal upload source.
if (asset.category !== 'personal_reference' && asset.category !== 'generated_result') {
...
```ts
const imageAsset = getOwnedAsset(userId, result.image_asset_id);
if (!imageAsset || imageAsset.category !== 'generated_result') {
