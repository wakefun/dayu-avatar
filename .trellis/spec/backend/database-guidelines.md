...
- Fine-tune derived tasks store `source_task_id` as the completed source task id and may store a `generated_result` asset id in the personal/source reference fields. Generic task creation must still only accept uploaded `personal_reference` source ids.
...
- `file_assets.content_hash` stores the SHA-256 hash of stored bytes and is returned as `contentHash` in asset/result/record payloads when available.
- `file_assets.deleted_at`, `generation_tasks.deleted_at`, and `gallery_items.deleted_at` are soft-delete markers; normal user-facing queries must filter them with `deleted_at IS NULL`.
- `gallery_items.generation_result_id` remains unique; removing from gallery marks `deleted_at`, and saving the same result again reactivates the existing row.
- Session rows store `session_data` JSON plus optional `user_id`, `auth_mode`, `expires_at`, and soft-delete `deleted_at`.
- `DAYU_DATA_ROOT` optionally overrides the runtime data root for isolated API tests; production/local app data defaults to `<repo>/data`.
...
- Fine-tune source result asset must be user-owned and category `generated_result` before creating the derived task.
- Result requested before task is completed -> `INVALID_STATE`.
- Active task deletion via records endpoint -> `409 INVALID_STATE`; callers must cancel active tasks with `POST /api/generation-tasks/:taskId/cancel`.
- Canceled or soft-deleted task lookup -> `404 NOT_FOUND` through normal owned-task endpoints.
- Removed gallery item lookup/download/avatar action -> `404 NOT_FOUND`.

### 5. Good/Base/Bad Cases

- Good: create upload asset row and write file under `data/uploads/<category>/<yyyy>/<mm>/...`, exposing `/static/uploads/...`.
- Good: compute asset content hash before writing; same user/category/hash returns the existing asset row, and same user/hash in another category creates an alias row that reuses the same static file path.
- Good: persist generated result/thumbnail assets through the same hash-aware storage boundary so duplicate generated bytes share one physical file.
...
- Good: create fine-tune tasks by reading `generation_results.image_asset_id` server-side and writing that generated asset id into the derived task source-reference fields with `source_task_id` set.
- Good: delete/cancel APIs set `deleted_at` and leave static files in place; record delete also soft-deletes related gallery rows and clears `saved_to_gallery`.
...
- Bad: do not physically delete `/data/uploads` or `/data/generated` files for user delete/cancel/gallery-remove flows.
- Bad: do not let canceled tasks appear in normal queue/history/records payloads, even if older rows have `deleted_at IS NULL`.
...
- Add a fine-tune smoke assertion that the derived task uses a generated result asset as its sole source reference, keeps source task generation params, and records `sourceTaskId`.
- Add deletion/dedupe smoke assertions that upload duplicates reuse hash/path, active task cancellation hides the task, record delete soft-deletes related gallery rows, removed gallery rows can be reactivated, and static files still exist after deletion.
