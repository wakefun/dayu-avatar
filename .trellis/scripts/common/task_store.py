...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    if "archived_to" in result:
        ...
        if not getattr(args, "no_commit", False):
            _auto_commit_archive(dir_name, repo_root, year_month, modified_children)
...
def _auto_commit_archive(
    ...
    plus any child task ``task.json`` files edited (parent → children
    relationship update). Dirty changes in OTHER active task dirs are
    NOT bundled into the archive commit.
    ...
    paths = safe_archive_paths_to_add(
        repo_root,
        task_name=task_name,
        archive_year_month=archive_year_month,
    ...
    if rc != 0:
        print(
            f"[WARN] git rm failed: {err.strip() if err else 'unknown error'}",
            file=sys.stderr,
        )
        return

    commit_paths = list(dict.fromkeys(paths))
    rc, source_changes, _ = run_git(
        ["diff", "--cached", "--name-only", "--", source_rel],
        cwd=repo_root,
    )
    if rc == 0 and source_changes.strip() and source_rel not in commit_paths:
